...
Keys are UTF-8 strings. Values are opaque bytes. CLI examples below use text
input, which is sent as UTF-8, but the transport and SDK also support arbitrary
binary payloads.
...
Writes a value with optional existence conditions, version-based compare-and-set,
expiration, TTL preservation, and previous-value return.
...
Numeric commands interpret the stored value as a signed integer encoded in UTF-8
text.
