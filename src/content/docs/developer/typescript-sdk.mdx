...
The official TypeScript SDK is published as [`@vaylix/client`](https://www.npmjs.com/package/@vaylix/client?activeTab=readme). The current SDK line is `0.3.x` and is aware of Vaylix server `0.5.0`.
...
- aligned with the current VTP2 transport, structured `EXEC` results, and the HA cluster command surface used by server `0.5.0`
...
The SDK exposes the server `0.5.0` operational inspection surface directly:
...
Those are for controlled administrative workflows. They are not ordinary application commands. In `0.5.0`, writes on non-leaders are rejected with typed server errors, while default reads should be sent to the leader unless the application explicitly accepts stale follower reads.
...
It is still intentionally narrower than the full Rust CLI, but it now covers the core application-facing and HA-aware operational surface shipped in server `0.5.0`.
