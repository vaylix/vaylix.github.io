...
The official TypeScript SDK is published as [`@vaylix/client`](https://www.npmjs.com/package/@vaylix/client?activeTab=readme). The current SDK line is `0.4.x` and is compatible with the Vaylix server `0.8.x` line.
...
## Binary Values and CAS

Vaylix `0.8.0` stores values as opaque bytes. The SDK keeps UTF-8 string
methods for convenience and exposes explicit byte APIs for binary-safe flows:
...
The structured `EXEC` transport payload in server `0.3.0` is what makes this
result shape reliable. In `0.8.0`, the typed payload also preserves exact byte
values through fields such as `valueBytes`.
...
It is still intentionally narrower than the full Rust CLI, but it covers the
core application-facing and HA-aware operational surface shipped in the current
`0.8.x` server line.
